...
use Odin\http\middleware\Middleware;
use Odin\routes\Route;
...
class Router
{
    ...
    /**
     * Objeto Odin\Route - Rota a ser despachada
    ...
    public function group($prefix, $fnc)
    {
        ...
        if ($fnc instanceof \Closure) {
            ...
            \Odin\utils\Errors::throwError('Argumento Inválido!', 'Callable do metodo group DEVE ser um Closure', 'bug');
        }
     ...
     * @param $routeName string
     * @return Odin\routes\Route
     */
    protected function getRoute($routeName)
    {
        $routeIndex = array_search($routeName, $this->routeNames);
        if ($routeIndex == false) {
            \Odin\utils\Errors::throwError('Rota não encontrada!', 'Rota ' . $routeName . ' não encontada', 'routenotfound');
    ...
    public function pathFor($routeName, $params = array())
    {
        if ($rota = $this->getRoute($routeName)) {
            ...
            if ($qtdParams > 0 && count($params) == 0) {
              \Odin\utils\Errors::throwError('Argumento Inválido!', 'A rota ' . $routeName . ' requer ' . $qtdParams . ' parametro(s)!', 'bug');
    ...
    public function redirectTo($routeName, $query = array(), $params = array())
    {
        ...
        if (!is_array($query)) {
            \Odin\utils\Errors::throwError('Argumento Inválido', 'Router::redirectTo A query deve ser um array!', 'bug');
    ...
    */
     ...
     * @param $container Odin\Container
     */
    public function execute(\Odin\routes\Container $container)
    {
